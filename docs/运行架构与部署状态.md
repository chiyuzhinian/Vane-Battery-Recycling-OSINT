...
| **实际在跑的是** | ✅ **纯 Python 采集层** | 9 个注册连接器（+ 浏览器通道）+ CLI 脚本 + 文件存储，不依赖任何容器 |
...
graph TB
    ...
    subgraph CONN["连接器层 · app/connectors（REGISTRY 注册 9 个）"]
...
| **B · 定向直采 Connector** | 权威源深度 | ✅ **主力**：REGISTRY 注册 9 个连接器，1,545 条；数据里共出现 14 种 `source_id`（含浏览器抓取与人工整理批次） |
...
| 钉钉告警 | 出口 | ❌ 未实现 |
| **`app/connectors/*`（注册 9 个）** | 通道 B | ✅ **已实现，是真正的产能所在** |
...
> 真正在跑的是 9 个注册连接器 + 浏览器通道 + 判定引擎 + 模板化报告，
